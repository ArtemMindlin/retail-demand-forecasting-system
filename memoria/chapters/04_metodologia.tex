...
\end{tikzpicture}
\shorthandon{>}%
\caption{Los dos niveles de monitorización, situados en el instante en que cada uno puede evaluarse. El nivel 1 compara distribuciones de entrada y no necesita esperar a nada; el nivel 2 mide el error y por tanto solo existe una vez transcurrido el horizonte. La franja sombreada es la ventana en la que el sistema ya ha decidido y todavía no puede saber si acertó, que es la limitación que la Sección~\ref{sec:contrato_operacional} enuncia como contrato. Ninguno de los dos promueve un reentrenamiento por sí solo.}
