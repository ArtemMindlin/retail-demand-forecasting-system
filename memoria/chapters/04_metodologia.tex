...
Este bloque recorre, en este orden, la cadena completa de decisiones que separan el histórico
de ventas de una recomendación de pedido: cómo se formula el problema y se corrige la censura
por rotura de stock antes de que llegue al modelo, qué información temporal se codifica en cada
fila del panel supervisado, cómo se entrena el modelo para producir cuantiles en lugar de un
único valor esperado, por qué se implementan dos \textit{backends} de \textit{Gradient Boosting}
en paralelo, cómo se corrige el cruce de cuantiles que introduce entrenarlos por separado, cómo
se buscan sus hiperparámetros bajo dos objetivos en conflicto, cómo se calibra la incertidumbre
resultante por categoría de producto, y bajo qué protocolo de \textit{backtesting} se mide todo
lo anterior sin fuga de información. Cierra el bloque la discusión del supuesto de costes
uniformes que sostiene la decisión final de inventario.
...
\begin{itemize}
    ...
    $\hat{g}$ sobre los registros históricos no afectados por stockouts, que aprende a
    predecir la demanda de un \textbf{día completo sin rotura}. El vector de covariables
    $x_{s,t}$ incluye: identificador de serie, mes, día de la semana, día del mes, media
    histórica de días limpios de la serie como prior, e indicadores contextuales
    disponibles en el dataset (descuento, festivo, temperatura, precipitación, humedad y
    viento). El ratio de stockout $h_{s,t}/16$ se excluye deliberadamente de $x_{s,t}$:
    entrenado únicamente con días limpios, donde ese ratio vale siempre 0, \textit{LightGBM}
    no puede dividir sobre una constante y la variable no aportaba ninguna importancia al
    modelo, pese a leerse en el código como si el regresor conociera la severidad del día
    que reconstruye.

    La severidad entra en cambio en la \textbf{reconciliación}, no en el regresor: puesto
    que $\hat{g}$ predice un día completo, la fracción del día perdida por la rotura es
    $r_{s,t} = \text{clip}(h_{s,t}/16,\ 0,\ 1)$, y la demanda latente se estima recomponiendo
    lo observado con esa fracción de lo predicho:
    \begin{equation}
        \tilde{y}_{s,t} = y_{s,t} + r_{s,t} \cdot \max\!\left( \hat{g}(x_{s,t}),\, 0 \right)
    \end{equation}
    Esta regla sustituye a un $\max(y_{s,t}, \hat{g}(x_{s,t}))$ inicial, que descartaba la
    venta observada en roturas leves --precisamente donde es la evidencia más fiable-- en
    favor de la predicción íntegra del modelo. Las estrategias \textit{Clipped Scaling} y
    \textit{Media Histórica Local} conservan esa regla de máximo, adecuada cuando no hay una
    predicción de día completo con la que recomponer proporcionalmente.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Lags de Demanda y Ventanas Deslizantes}: Para capturar la autocorrelación de la serie, se calculan lags de la demanda observada en los instantes $t-1$, $t-7$ y $t-14$. La elección no es arbitraria: el análisis de la función de autocorrelación (ACF) del Capítulo~\ref{chap:analisis_datos} reveló picos significativos en los retardos 7 y 14, correspondientes a la estacionalidad semanal dominante del dataset. El lag $t-1$ captura inercia de corto plazo (efecto del día anterior); $t-7$ y $t-14$ recogen la estacionalidad semanal dominante detectada en el ACF; un cuarto retardo a $t-28$, que aproximaba el ciclo mensual de reposición, se retiró del conjunto final: con 96 días de historia disponible su coste no está en el número de columnas sino en el arranque en frío, porque el primer origen predecible de cada serie se retrasa hasta que el retardo más largo tiene dato: 28 días en lugar de 14, lo que cuesta catorce orígenes por serie. Sobre ventanas de 7 y 14 días se calculan además la media, la suma y la desviación estándar rodantes. Todos los estadísticos se computan con desplazamiento de un día (\texttt{shift(1)}) para garantizar ausencia de \textit{data leakage}.
...
De las tres implementaciones de referencia que revisa el Capítulo~\ref{chap:estado_del_arte}, el
sistema implementa \textbf{dos}: LightGBM y CatBoost, ambas bajo el mismo protocolo de
\textit{backtesting} y el mismo envoltorio cuantílico. XGBoost se descarta antes de
implementarse, y conviene ser preciso sobre el motivo, porque no es una limitación de la
herramienta: desde su versión 1.5 soporta variables categóricas de forma nativa, con un
algoritmo de particionamiento propio para las divisiones categóricas, de modo que el
\textit{one-hot} manual dejó de ser necesario. La razón es de alcance y de redundancia. Sobre el
eje que más pesa en este dominio, la forma de codificar identificadores de cardinalidad alta,
XGBoost y LightGBM comparten mecanismo: ambos buscan la partición ordenando las categorías por
el estadístico del gradiente. Un tercer \textit{backend} que replica el mecanismo de uno de los
dos ya implementados multiplicaría cada corrida de \textit{backtesting} sin añadir un contraste
nuevo, mientras que CatBoost es el único de los tres que aporta un mecanismo distinto.

Entre las dos que sí se implementan, la elección del campeón \textbf{no se decide con argumentos
de diseño} sino empíricamente, por el criterio de promoción de la
Sección~\ref{sec:promocion}. Esa distinción importa, porque el orden por error puntual y el
orden por coste no coinciden, y reportarlos por separado es una de las conclusiones del trabajo.
Las propiedades de las dos implementaciones explican \emph{por qué es razonable} que gane la que
gana, no la razón por la que se selecciona: los estadísticos de destino ordenados de CatBoost
son la única defensa explícita frente a la fuga por la etiqueta cuando los identificadores
tienen la cardinalidad de este catálogo, y sus árboles \textit{oblivious} actúan como
regularizador estructural sobre un panel donde muchas series tienen historia corta.

La estimación nativa de incertidumbre que CatBoost ofrece \textbf{no se explota} en este sistema:
la cuantificación de la incertidumbre se obtiene de la rejilla de cuantiles y de la calibración
conformal, comunes a los dos \textit{backends}, de modo que la comparación entre ellos se
mantiene en igualdad de condiciones.
