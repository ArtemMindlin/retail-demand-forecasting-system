...
\paragraph{La vía de respaldo no se ejercita en el panel experimental.} Conviene declararlo aquí y no dejar que se deduzca de su ausencia en el Capítulo~\ref{chap:resultados}. La construcción del panel exige un mínimo de $70$ días de historia por serie (\texttt{min\_}\allowbreak\texttt{history\_}\allowbreak\texttt{days}), de modo que ninguna referencia del universo experimental puede quedarse sin sus retardos más largos: en la corrida a escala, las $10.500$ observaciones de validación se resuelven por la vía del modelo y la cascada no se activa una sola vez. La vía existe para el despliegue, donde el surtido incorpora referencias nuevas de forma continua, y su corrección se verifica mediante pruebas sobre paneles sintéticos en lugar de mediante la corrida. Declararlo importa porque la magnitud que el párrafo anterior propone auditar, la proporción de recomendaciones por nivel de la cascada, vale exactamente cero en los resultados que este trabajo reporta, y ese cero mide el diseño del panel, no la robustez del mecanismo.
