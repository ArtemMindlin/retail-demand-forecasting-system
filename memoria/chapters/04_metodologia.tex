...
Este procedimiento es computacionalmente ligero ($\mathcal{O}(M \log M)$, equivalente a ordenar con \texttt{np.sort} sobre el eje de cuantiles) y posee una propiedad teórica fundamental \cite{chernozhukov2010quantile}: el \textit{rearrangement} reduce (o mantiene) la pérdida cuantílica integrada sobre todos los niveles de cuantil respecto a las estimaciones originales. La Figura~\ref{fig:quantile_crossing} contrasta la rejilla cruda con la reordenada.

\begin{figure}[H]
\centering
\shorthandoff{>}%
\begin{tikzpicture}[xscale=1.0, yscale=0.78,
    pt/.style={circle, fill=blue!70, draw=blue!85, inner sep=1.4pt}]
  % Panel (a): rejilla cruda
  \draw[->,thick] (-0.1,0) -- (5.1,0) node[right,font=\sffamily\scriptsize] {$q$};
  \draw[->,thick] (0,-0.15) -- (0,5.3) node[above,font=\sffamily\scriptsize] {$\hat{y}_q$};
  \foreach \x/\q in {0.7/0{,}1, 1.6/0{,}3, 2.5/0{,}5, 3.4/0{,}7, 4.3/0{,}9} {
    \node[below,font=\sffamily\scriptsize] at (\x,-0.15) {$\q$};
    \draw[gray!30] (\x,0) -- (\x,4.9);
  }
  \draw[blue!70,thick] (0.7,1.2) -- (1.6,2.1) -- (2.5,2.9) -- (3.4,4.3);
  \draw[red!75,very thick] (3.4,4.3) -- (4.3,3.5);
  \foreach \x/\y in {0.7/1.2, 1.6/2.1, 2.5/2.9} { \node[pt] at (\x,\y) {}; }
  \node[circle, fill=red!70, draw=red!85, inner sep=1.4pt] at (3.4,4.3) {};
  \node[circle, fill=red!70, draw=red!85, inner sep=1.4pt] at (4.3,3.5) {};
  \node[font=\sffamily\scriptsize, red!80] at (3.85,5.05) {cruce};
  \draw[red!70,thin] (3.85,4.85) -- (3.9,4.0);
  \node[font=\sffamily\scriptsize] at (2.5,-1.0) {(a) rejilla cruda};

  % Flecha de reordenacion
  \draw[->,very thick,gray!70] (5.45,2.4) -- (6.55,2.4);
  \node[above,font=\sffamily\scriptsize,gray] at (6.0,2.5) {\texttt{sort}};

  % Panel (b): tras el rearrangement
  ...
  \draw[blue!70,thick] (0.7,1.2) -- (1.6,2.1) -- (2.5,2.9) -- (3.4,3.5) -- (4.3,4.3);
  \foreach \x/\y in {0.7/1.2, 1.6/2.1, 2.5/2.9} { \node[pt] at (\x,\y) {}; }
  \node[circle, fill=orange!80, draw=orange!90, inner sep=1.4pt] at (3.4,3.5) {};
  \node[circle, fill=orange!80, draw=orange!90, inner sep=1.4pt] at (4.3,4.3) {};
  \draw[orange!85,thin,dashed] (3.4,4.3) -- (4.3,4.3);
  \draw[orange!85,thin,dashed] (3.4,3.5) -- (4.3,3.5);
  \node[font=\sffamily\scriptsize, orange!85] at (3.85,5.05) {intercambiados};
  \node[font=\sffamily\scriptsize] at (2.5,-1.0) {(b) tras el \textit{rearrangement}};
  \end{scope}
\end{tikzpicture}
\shorthandon{>}%
\caption[Cruzamiento de cuantiles y su corrección]{Cruzamiento de cuantiles y su corrección. En (a) los estimadores se entrenan de forma
independiente y el del cuantil $0{,}7$ predice por encima del de $0{,}9$ (segmento rojo), lo que hace
que la rejilla deje de ser una distribución acumulada válida. En (b) el \textit{rearrangement} devuelve
la monotonicidad limitándose a intercambiar las posiciones de los dos valores en conflicto (naranja):
ningún valor predicho se modifica, solo su asignación a un nivel de cuantil.}
\label{fig:quantile_crossing}
\end{figure}
...
\medskip
\noindent\textbf{Ejemplo numérico.} Supóngase que el modelo predice el intervalo $[10, 20]$ unidades para la demanda acumulada (amplitud $= 10$, $\alpha = 0{,}2$). La Figura~\ref{fig:winkler_score} representa la función completa y sitúa sobre ella los tres casos:
...
\centering
\shorthandoff{>}%
\begin{tikzpicture}[x=0.26cm, y=0.032cm]
  % Banda del intervalo predicho
  \fill[blue!8] (10,0) rectangle (20,122);
  % Rejilla horizontal
  \foreach \v in {10,60,110} {
    \draw[gray!35,dashed] (0,\v) -- (32,\v);
    \node[left,font=\sffamily\scriptsize,gray] at (-0.6,\v) {\v};
  }
  % Ejes
  \draw[->,thick] (-0.5,0) -- (33.5,0) node[right,font=\sffamily\scriptsize] {$y$ real};
  \draw[->,thick] (0,-4) -- (0,132) node[above,font=\sffamily\scriptsize] {$W$};
  % Curva de la funcion a trozos
  \draw[blue!70,very thick] (0,110) -- (10,10) -- (20,10) -- (30,110);
  % Marcas del eje x
  \foreach \x in {5,15,25} {
    \draw[gray!60] (\x,0) -- (\x,-3);
    \node[below,font=\sffamily\scriptsize] at (\x,-3) {\x};
  }
  \draw[gray!60] (10,0) -- (10,-3);
  \node[below,font=\sffamily\scriptsize] at (10,-3) {$L=10$};
  \draw[gray!60] (20,0) -- (20,-3);
  \node[below,font=\sffamily\scriptsize] at (20,-3) {$U=20$};
  % Los tres casos del ejemplo
  \node[circle,fill=orange!85,draw=orange!95,inner sep=1.4pt] at (15,10) {};
  \node[circle,fill=orange!85,draw=orange!95,inner sep=1.4pt] at (5,60) {};
  \node[circle,fill=orange!85,draw=orange!95,inner sep=1.4pt] at (25,60) {};
  \node[above=2pt,font=\sffamily\scriptsize] at (15,10) {Caso 1};
  \node[above=2pt,font=\sffamily\scriptsize] at (5,60) {Caso 2};
  \node[above=2pt,font=\sffamily\scriptsize] at (25,60) {Caso 3};
  % Anotaciones
  \node[font=\sffamily\scriptsize,gray!80,align=center] at (15,52)
    {pago base\\ $U-L$};
  \draw[gray!60,thin] (15,41) -- (15,26);
  \node[font=\sffamily\scriptsize,gray!80,rotate=-51] at (7.5,78) {pendiente $2/\alpha=10$};
  \node[font=\sffamily\scriptsize,gray!80,rotate=51] at (22.5,78) {pendiente $2/\alpha=10$};
  \node[font=\sffamily\scriptsize,blue!70] at (15,128) {intervalo predicho};
\end{tikzpicture}
\shorthandon{>}%
\caption[Winkler Score como función de la demanda observada]{Winkler Score en función de la demanda observada, para el intervalo predicho $[10, 20]$
con $\alpha = 0{,}2$. Dentro del intervalo la puntuación es constante e igual a la amplitud, de modo que
la única forma de mejorarla es estrechar la banda; fuera de él crece con pendiente $2/\alpha = 10$, lo que
penaliza el fallo de cobertura muy por encima de lo que cuesta ensanchar. Los tres puntos naranjas son
los casos del ejemplo numérico.}
...
categoría no haya aparecido en el set de calibración. El procedimiento completo, resumido en la
Figura~\ref{fig:mondrian_conformal}, es el siguiente:
...
\begin{tikzpicture}[
  ...
  box/.style={draw=blue!60, rounded corners=3pt, fill=blue!8, inner sep=5pt},
  grp/.style={draw=purple!70, rounded corners=3pt, fill=purple!10, text width=2.3cm,
              minimum height=1.5cm, inner sep=4pt},
  fb/.style={draw=gray!70, dashed, rounded corners=3pt, fill=gray!10, text width=2.3cm,
             minimum height=1.5cm, inner sep=4pt},
  arr/.style={->, thick, gray!65}
]
  \node[box, text width=6.6cm] (cal) at (0,5.9)
    {Conjunto de calibración\\ \footnotesize últimos 14 días del \textit{train}, no usados en el ajuste};
  \node[box, text width=8.0cm] (score) at (0,4.2)
    {Score de no conformidad CQR por observación\\[2pt]
     \mbox{$s_i = \max\!\left(\hat{y}_{P10}(x_i) - y_i,\ y_i - \hat{y}_{P90}(x_i)\right)$}};
  \node[gray!75] (part) at (0,2.95) {partición por \texttt{third\_category\_id}};

  \node[grp] (g1) at (-4.3,1.6) {Categoría $g_1$\\[2pt] $\hat{q}_{g_1}$ bajo\\[1pt] \footnotesize estable};
  \node[grp] (g2) at (-1.45,1.6) {Categoría $g_2$\\[2pt] $\hat{q}_{g_2}$ medio\\[1pt] \footnotesize regular};
  \node[grp] (g3) at ( 1.4,1.6) {Categoría $g_3$\\[2pt] $\hat{q}_{g_3}$ alto\\[1pt] \footnotesize intermitente};
  \node[fb]  (fb) at ( 4.25,1.6) {$\hat{q}_{\text{global}}$\\[2pt] \footnotesize respaldo si $n_g = 0$ o la categoría es nueva};

  \node[box, text width=9.0cm] (out) at (0,-0.9)
    {Intervalo ajustado, un ensanchamiento propio por familia\\[2pt]
     \mbox{$\left[\max(\hat{y}_{P10} - \hat{q}_g,\ 0),\ \hat{y}_{P90} + \hat{q}_g\right]$}\\[2pt]
     \footnotesize cobertura del 80\% \emph{dentro} de cada categoría, no solo en media};

  \draw[arr] (cal) -- (score);
  \draw[arr] (score) -- (part);
  \draw[arr] (part) -- (g1.north);
  \draw[arr] (part) -- (g2.north);
  \draw[arr] (part) -- (g3.north);
  \draw[arr, dashed] (part) -- (fb.north);
  \draw[arr] (g1.south) -- (g1.south |- out.north);
  \draw[arr] (g2.south) -- (g2.south |- out.north);
  \draw[arr] (g3.south) -- (g3.south |- out.north);
  \draw[arr, dashed] (fb.south) -- (fb.south |- out.north);
\end{tikzpicture}
\shorthandon{>}%
\caption[Calibración Mondrian Conformal por categoría]{Calibración \textit{Mondrian Conformal}. Los scores de no conformidad calculados sobre la
ventana de calibración se agrupan por categoría de producto, y cada grupo obtiene su propio factor
corrector $\hat{q}_g$ en lugar de compartir uno global. Así, las familias con demanda dispersa reciben
bandas más anchas y las estables no pagan por ellas, que es lo que un $\hat{q}$ único enmascara. El
factor global se conserva únicamente como respaldo (trazo discontinuo) para categorías ausentes de la
ventana de calibración.}
\label{fig:mondrian_conformal}
\end{figure}
...
\end{equation}
y el sistema adopta $\lambda = 0{,}5$. La lectura operativa del valor es directa: un \textit{challenger} un $2\%$ más caro necesita un $4\%$ mejor de Winkler para promocionar, magnitudes ambas dentro del ruido; uno un $10\%$ más caro necesitaría un $20\%$, y uno un $20\%$ más caro un $40\%$, exigencia que en la práctica ningún modelo satisface. El coste real, en suma, no se vende barato, y la calibración solo arbitra donde el coste ya no informa. La Figura~\ref{fig:region_promocion} representa esa frontera y la banda de ruido que la acota.

\begin{figure}[H]
\centering
\shorthandoff{>}%
\begin{tikzpicture}[x=0.155cm, y=0.062cm]
  % Region que supera el primer guardarrail (S > 0)
  \fill[blue!8] (-22,44) -- (11,-22) -- (22,-22) -- (22,46) -- (-22,46) -- cycle;
  % Banda de ruido del coste
  \draw[gray!70, dashed] (-8.3,-15) -- (-8.3,46);
  \draw[gray!70, dashed] (8.3,-15) -- (8.3,46);
  \draw[<->, gray!75] (-8.3,-17) -- (8.3,-17);
  \node[below,font=\sffamily\scriptsize,gray!85] at (0,-17.5) {ruido del coste ($\pm 8{,}3$)};
  % Ejes
  \draw[->,thick] (-23,0) -- (23.5,0) node[right,font=\sffamily\scriptsize] {$\Delta_{\text{cost}}$};
  \draw[->,thick] (0,-23) -- (0,48) node[above,font=\sffamily\scriptsize] {$\Delta_{\text{wink}}$};
  \foreach \x in {-20,-10,10,20} {
    \draw[gray!60] (\x,0) -- (\x,-1.6);
    \node[below,font=\sffamily\scriptsize,gray] at (\x,-1.6) {\x};
  }
  \foreach \yv in {20,40} {
    \draw[gray!60] (0,\yv) -- (-0.7,\yv);
    \node[left,font=\sffamily\scriptsize,gray] at (-0.9,\yv) {\yv};
  }
  % Frontera S = 0
  \draw[blue!75,very thick] (-22,44) -- (11,-22);
  % Brecha de Winkler observada
  \draw[orange!85,thick,dashed] (-23,14.8) -- (23,14.8);
  \node[right,font=\sffamily\scriptsize,orange!85] at (0.8,17.6)
    {brecha de Winkler observada (14,8)};
  % Puntos de ejemplo sobre la frontera
  \foreach \px/\py in {-2/4, -10/20, -20/40} {
    \node[circle,fill=blue!80,draw=blue!90,inner sep=1.3pt] at (\px,\py) {};
  }
  \draw[gray!60,thin] (0.7,6.5) -- (-1.5,4.4);
  \node[right,font=\sffamily\scriptsize] at (0.8,6.5) {$(-2,\ +4)$};
  \node[left=2pt,font=\sffamily\scriptsize] at (-10.6,20) {$(-10,\ +20)$};
  \node[right=1pt,font=\sffamily\scriptsize] at (-18.5,43) {$(-20,\ +40)$};
  % Corte de la frontera con la brecha observada
  \node[circle,fill=orange!90,draw=orange!95,inner sep=1.3pt] at (-7.4,14.8) {};
  \draw[gray!65,thin] (-7.6,14.2) -- (-9.2,11);
  \node[left,font=\sffamily\scriptsize] at (-9.3,11) {$-7{,}4$};
  % Etiquetas de region
  \node[font=\sffamily\scriptsize,blue!70,align=center] at (14,34) {promociona\\ $S > 0$};
  \node[font=\sffamily\scriptsize,gray!75,align=center] at (-16,-13) {no promociona\\ $S < 0$};
\end{tikzpicture}
\shorthandon{>}%
\caption[Región de promoción en el plano coste--Winkler]{Primer guardarraíl de promoción en el plano de mejoras relativas, con $\lambda = 0{,}5$.
La frontera $S = \Delta_{\text{cost}} + \lambda\,\Delta_{\text{wink}} = 0$ tiene pendiente $-2$: cada punto
de coste sacrificado exige dos de Winkler. Las verticales discontinuas acotan el semiancho del ruido
del \textit{bootstrap} ($\pm 8{,}3$ pp), dentro del cual el experimento no distingue dos modelos por coste. El corte de la
frontera con la brecha de Winkler observada entre \textit{backends} (línea naranja, $14{,}8$ pp) cae
\emph{dentro} de esa banda: el mayor desplazamiento de coste que la calibración puede compensar es
$\lambda \cdot 14{,}8 = 7{,}4$ pp, menor que el ruido, de modo que con este $\lambda$ el Winkler Score
solo decide donde el coste ya no informa y nunca revierte una diferencia de coste real. Los tres puntos
azules sobre la frontera son los ejemplos del texto, en coordenadas
$(\Delta_{\text{cost}}, \Delta_{\text{wink}})$: un \textit{challenger} un $2\%$ más caro es
$\Delta_{\text{cost}} = -2$ y necesita $+4$ de Winkler para compensarlo. La frontera dibujada es la de
$\theta_{\text{cost}} = 0$; un umbral positivo la desplaza en paralelo hacia la región de promoción.}
\label{fig:region_promocion}
\end{figure}
...
\end{equation}
es decir, la mediana predicha más tres cuartas partes de la semianchura superior del intervalo conformal. La Figura~\ref{fig:fractil_interpolacion} representa la lectura geométrica de esta regla. Este es el mecanismo concreto por el que el ensanchamiento por categoría de la calibración Mondrian (Sección~\ref{sec:conformal}) se traduce en un colchón de seguridad adaptativo: una categoría con mayor dispersión empírica recibe un $\hat{Q}(0{,}9)$ más alto y, con ello, un pedido proporcionalmente mayor, sin que ninguna regla adicional lo estipule.

\begin{figure}[H]
\centering
\shorthandoff{>}%
\begin{tikzpicture}[x=7.4cm, y=0.155cm]
  % Zona de saturacion
  \fill[gray!18] (0.9,0) rectangle (1.03,27);
  % Ejes
  \draw[->,thick] (0,0) -- (1.07,0) node[right,font=\sffamily\scriptsize] {$\tau$};
  \draw[->,thick] (0,0) -- (0,28.5) node[above,font=\sffamily\scriptsize] {unidades};
  \foreach \t/\lab in {0.1/0{,}1, 0.5/0{,}5, 0.9/0{,}9} {
    \draw[gray!55,dashed] (\t,0) -- (\t,26);
    \node[below,font=\sffamily\scriptsize,gray] at (\t,-0.4) {$\lab$};
  }
  \foreach \v in {6,12,22} {
    \draw[gray!45,dotted] (0,\v) -- (0.92,\v);
    \node[left,font=\sffamily\scriptsize,gray] at (-0.006,\v) {\v};
  }
  % Funcion cuantil calibrada
  \draw[blue!75,very thick] (0.1,6) -- (0.5,12) -- (0.9,22);
  \draw[blue!60,thick,dashed] (0.9,22) -- (1.03,22);
  \foreach \t/\v in {0.1/6, 0.5/12, 0.9/22} {
    \node[circle,fill=blue!80,draw=blue!90,inner sep=1.5pt] at (\t,\v) {};
  }
  \node[above,font=\sffamily\scriptsize,blue!75] at (0.3,13.5) {$\hat{Q}_{s,t}(\tau)$ calibrada};
  % Fractil critico y pedido resultante
  \draw[orange!85,thick,dashed] (0.8,0) -- (0.8,19.5) -- (0,19.5);
  \node[circle,fill=orange!90,draw=orange!95,inner sep=1.5pt] at (0.8,19.5) {};
  \node[below,font=\sffamily\scriptsize,orange!85] at (0.8,-3.6) {$q^\star = 0{,}80$};
  \node[left,font=\sffamily\scriptsize,orange!85] at (-0.006,19.5) {$19{,}5$};
  \node[right,font=\sffamily\scriptsize,orange!85] at (0.07,25.8)
    {\mbox{$O_{s,t} = \hat{Q}(0{,}5) + 0{,}75\cdot[\hat{Q}(0{,}9) - \hat{Q}(0{,}5)]$}};
  % Saturacion
  \draw[gray!75,thin,dotted] (0.909,0) -- (0.909,22);
  \node[font=\sffamily\scriptsize,gray!80,align=center,rotate=90] at (0.968,13)
    {satura: no extrapola};

\end{tikzpicture}
\shorthandon{>}%
\caption[Interpolación de la función cuantil en el fractil crítico]{Traducción de la distribución calibrada en cantidad de pedido. La rejilla estimada
$\mathcal{Q} = \{0{,}1;\ 0{,}5;\ 0{,}9\}$ no contiene el fractil crítico $q^\star = 0{,}80$, de modo que
el pedido se lee interpolando linealmente entre los dos niveles que lo rodean, a tres cuartas partes del
tramo. Como la interpolación se hace sobre la función cuantil \emph{calibrada}, el pedido hereda la
garantía conformal. A la derecha de $0{,}9$ (zona gris) la regla no puede extrapolar: un ratio $10{:}1$
implicaría $q^\star \approx 0{,}909$ y el valor satura en $\hat{Q}(0{,}9)$.}
\label{fig:fractil_interpolacion}
\end{figure}
...
La cascada recorre de lo específico a lo general y se detiene en el primer nivel con dato, de modo que un producto nuevo en una tienda hereda el comportamiento de ese mismo producto en el resto de la red antes de recurrir a su categoría. Las medias se calculan exclusivamente sobre la historia disponible en el origen de predicción (el panel de entrenamiento, o las filas estrictamente anteriores a la fecha de decisión cuando la llamada procede de la simulación en \textit{streaming}), de modo que la vía de respaldo respeta el mismo contrato causal que la vía del modelo. La Figura~\ref{fig:cascada_respaldo} resume el enrutado completo.
...
\begin{tikzpicture}[
  every node/.style={font=\sffamily\scriptsize, align=center},
  box/.style={draw=blue!60, rounded corners=3pt, fill=blue!8, inner sep=5pt},
  lvl/.style={draw=purple!70, rounded corners=3pt, fill=purple!10, text width=1.85cm,
              minimum height=0.95cm, inner sep=3pt},
  arr/.style={->, thick, gray!65},
  tag/.style={font=\sffamily\scriptsize, gray!75, inner sep=1.5pt}
]
  \node[box, text width=5.0cm] (fila) at (0,4.4) {Fila del marco de inferencia};
  \node[box, draw=purple!70, fill=purple!8, text width=3.1cm] (resp) at (-3.3,2.7)
    {Vía de respaldo\\ \footnotesize sin modelo ni intervalo};
  \node[box, text width=3.5cm] (mod) at (3.3,2.7)
    {Vía del modelo\\ \footnotesize campeón conformal, con intervalo};

  \node[lvl] (l1) at (-4.05,0.85) {serie};
  \node[lvl] (l2) at (-1.65,0.85) {producto};
  \node[lvl] (l3) at ( 0.75,0.85) {categoría};
  \node[lvl] (l4) at ( 3.15,0.85) {global};

  \node[box, draw=purple!70, fill=purple!8, text width=8.2cm] (out) at (-0.45,-1.2)
    {\mbox{$\hat{Y}^{\text{resp}}_{s,t} = H \cdot \overline{y}_{\ell(s)}$}\\
     \footnotesize media del primer nivel con historia, escalada al horizonte};

  \draw[arr] (fila) -- (resp) node[tag, midway, above left=-1pt] {algún nulo};
  \draw[arr] (fila) -- (mod)  node[tag, midway, above right=-1pt] {sin nulos};
  \draw[arr] (resp) -- (l1);
  \draw[arr] (l1) -- (l2);
  \draw[arr] (l2) -- (l3);
  \draw[arr] (l3) -- (l4);
  \node[tag] at (-0.45,1.75) {si el nivel no tiene historia, se pasa al siguiente};
  \foreach \n in {l1,l2,l3,l4} { \draw[arr] (\n.south) -- (\n.south |- out.north); }
\end{tikzpicture}
\shorthandon{>}%
\caption[Enrutado de cobertura y cascada de respaldo jerárquico]{Enrutado de cada fila del marco de inferencia. Una sola característica nula basta para
desviar la fila a la vía de respaldo, porque un retardo nulo imputado a cero es indistinguible, para un
modelo de \textit{boosting}, de una serie que no vendió. La vía de respaldo recorre la jerarquía de lo
específico a lo general y se detiene en el primer nivel con historia, de modo que un producto nuevo en
una tienda hereda su propio comportamiento en el resto de la red antes de recurrir a la categoría.}
\label{fig:cascada_respaldo}
\end{figure}
