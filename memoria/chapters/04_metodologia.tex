...
\node[blk, right=of inv] (ops) {\textbf{Bloque II}\\ Operación\\ \footnotesize Decidir y vigilar};
...
Tras cada ejecución del modo \texttt{experiment}, el sistema evalúa si alguno de los modelos candidatos (\textit{challengers}) debe reemplazar al campeón registrado. La comparación se realiza sobre el \texttt{cost\_summary} del protocolo \textit{walk-forward} (Sección~\ref{sec:backtesting}), excluyendo el \textit{holdout} externo para no mezclar dos poblaciones de filas distintas. La promoción no se decide por una única métrica, sino por tres guardarraíles que deben cumplirse simultáneamente:

\begin{enumerate}
    \item \textbf{Mejora de coste mínima} ($\Delta_{\text{cost}}$): el \textit{challenger} debe reducir el coste de inventario simulado respecto al campeón actual en al menos un umbral configurable $\theta_{\text{cost}}$, donde
...
El tercer guardarraíl existe porque los dos primeros no bastan. El umbral de mejora de coste es configurable y su valor por defecto es cero, de modo que por sí solo admite cualquier mejora por pequeña que sea: la comparación entre los dos \textit{backends} de este trabajo se decidía por una diferencia de coste del orden de la centésima de punto porcentual, magnitud que está dentro del propio ruido del experimento. Exigir evidencia estadística traslada a esta decisión el mismo criterio que el resto del trabajo aplica a cualquier comparación entre alternativas.
...
\paragraph{Refresco del modelo (\texttt{retrain}).} El reentrenamiento consolida todos los \textit{splits} disponibles en un único panel, aplica la misma estrategia de reconstrucción de censura que el experimento que seleccionó al campeón (Sección~\ref{sec:demanda_censurada}) y ajusta un solo modelo, sin evaluación por \textit{folds}. Tres propiedades hacen que este ajuste sea homólogo al del \textit{backtesting} y no un procedimiento distinto:
...
\paragraph{Emisión de recomendaciones (\texttt{score\_daily}).} El modo diario no reentrena: carga el artefacto conformal persistido y construye un \textbf{marco de inferencia} con \emph{una sola fila por serie}, correspondiente al último origen disponible de cada una. Esta fila tiene una propiedad que la distingue estructuralmente de cualquier fila del Bloque I: \textbf{no tiene etiqueta}. La demanda acumulada del horizonte que se está prediciendo es, por definición, futura, y el marco de inferencia no la contiene.

De ahí se sigue el contrato de este bloque, y merece enunciarse explícitamente porque condiciona todo lo que viene después:
...
\end{equation}
que no pertenece a $\mathcal{Q}$. La cantidad de pedido se obtiene interpolando linealmente la función cuantil calibrada entre los dos niveles estimados que rodean a $q^\star$:

\begin{equation}
\label{eq:order_quantity}
    Q_{s,t} = \max\!\left(0,\ \hat{Q}_{s,t}(\tau_k) + \frac{q^\star - \tau_k}{\tau_{k+1} - \tau_k}\Big[\hat{Q}_{s,t}(\tau_{k+1}) - \hat{Q}_{s,t}(\tau_k)\Big]\right),
...
\begin{equation}
    Q_{s,t} = \max\!\left(0,\ \hat{Q}_{s,t}(0{,}5) + 0{,}75 \cdot \big[\hat{Q}_{s,t}(0{,}9) - \hat{Q}_{s,t}(0{,}5)\big]\right),
\end{equation}
...
\paragraph{Por qué se interpola en lugar de entrenar en $q^\star$.} Entrenar un cuarto cuantil en $0{,}80$ habría evitado la interpolación, pero desacopla la decisión de la calibración: los tres cuantiles actuales sirven simultáneamente al intervalo de predicción ($0{,}1$--$0{,}9$) y a la decisión, y son los tres que la capa conformal corrige. Con la interpolación, además, \textbf{un cambio en la asimetría de costes no exige reentrenar}: basta reevaluar la Ecuación~\ref{eq:order_quantity} con otro $q^\star$ sobre las mismas predicciones. Es esta propiedad la que hace posible el análisis de sensibilidad al ratio $c_u/c_o$ que reporta el Capítulo~\ref{chap:resultados}, en el que las predicciones permanecen fijas y solo cambia la regla de decisión.
...
\paragraph{Alcance de la decisión.} La Ecuación~\ref{eq:order_quantity} resuelve un problema de \emph{un solo periodo} por origen de predicción. No hay posición de inventario que descontar, ni arrastre de existencias entre orígenes consecutivos, ni coste fijo de pedido: cada fecha de decisión se resuelve de forma independiente. Como se argumentó en la Sección~\ref{sec:newsvendor}, esto basta para ordenar políticas por su coste y su nivel de servicio, que es el uso que este trabajo da a la magnitud, pero no reproduce la dinámica de una política $(s, S)$ real. Es la limitación de alcance más relevante del bloque y se recoge como tal en las conclusiones.
...
Dos consecuencias metodológicas merecen enunciarse. La primera es que el nivel empleado se registra fila a fila, lo que convierte una degradación silenciosa en un dato auditable: la proporción de recomendaciones que salen de cada nivel de respaldo es una medida directa de cuánta cobertura del surtido descansa realmente en el modelo. La segunda es que \textbf{una fila de respaldo no lleva intervalo conformal}: la cascada produce un valor central, no una distribución, de modo que la garantía de cobertura de la Sección~\ref{sec:conformal} no la ampara y el fractil crítico no puede aplicársele. Por eso estas filas no se entregan como una recomendación más, sino marcadas para revisión humana, que es el objeto de la sección siguiente.
...
\paragraph{Umbrales relativos, no absolutos.} Las dos últimas banderas se definen sobre cuantiles de la \emph{distribución empírica de la propia ejecución} y no sobre constantes en unidades. La razón es que ni la anchura del intervalo ni la cantidad de pedido tienen una escala estable: dependen del surtido incluido, del régimen de demanda del periodo y de la asimetría de costes configurada, y un umbral absoluto calibrado sobre una corrida deja de tener sentido en la siguiente. Definirlos por cuantil fija en cambio la magnitud que de verdad importa a efectos operativos, que es \textbf{el tamaño del listado de excepciones}: estas dos causas aportan por construcción en torno al $5$--$6\%$ del catálogo, con independencia de la escala del panel o del régimen del periodo. Las dos primeras banderas no admiten esa acotación ---\texttt{cold\_start} depende de cuántas referencias carezcan de historia suficiente, y \texttt{drift\_watch} marca un tramo temporal completo cuando se dispara---, lo que es coherente con su naturaleza: señalan un fallo de cobertura o de validez, y su volumen es en sí mismo la información relevante. La contrapartida, que conviene no ocultar, es que estas dos banderas son intrínsecamente relativas: en una corrida en la que todas las predicciones fueran igual de inciertas, seguirían señalando un $5\%$ de ellas.
...
La Sección~\ref{sec:contrato_operacional} estableció que el plano operacional no puede medir su propio error en el momento de decidir. La monitorización es la respuesta a esa limitación, y se articula en dos niveles que vigilan objetos distintos y responden a preguntas distintas.
...
donde $p^{\text{ref}}_i$ y $p^{\text{act}}_i$ son las proporciones de observaciones que caen en el intervalo $i$ de una discretización en $B = 10$ tramos. Tres decisiones de cálculo condicionan la interpretación de la cifra. Primera, los bordes de los tramos son los \textbf{deciles de la distribución de referencia}, no una rejilla equiespaciada: sobre variables de demanda fuertemente asimétricas, una rejilla uniforme concentraría casi toda la masa en un tramo y el índice perdería resolución justo donde la tiene el fenómeno. Segunda, la ventana de referencia es la mitad \emph{más antigua} de las observaciones y la actual la mitad más reciente, con el corte en la fecha mediana: la deriva se mide así ``desde el entrenamiento'', que es la pregunta operativa. Tercera, el índice se reporta únicamente para las variables de mayor importancia según los valores SHAP del modelo explicado, porque una deriva en una variable que el modelo apenas usa no es un riesgo operativo sino ruido en el panel de alertas.
