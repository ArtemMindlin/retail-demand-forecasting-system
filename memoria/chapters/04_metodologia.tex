...
Esta elección tiene una consecuencia que conviene anticipar aquí, porque condiciona la lectura de los resultados del Capítulo~\ref{chap:resultados}: $\hat{y}$ \textbf{no es una estimación de la mediana ni de la media condicional}, sino del percentil 80. Toda métrica de error puntual calculada sobre él (el MAE y el RMSE que reporta la Tabla~\ref{tab:metrics_predictive}) mide, por tanto, un estimador distinto del que esas métricas premian. El estimador puntual queda además fuera de la corrección conformal, que actúa sobre los extremos del intervalo, motivo por el cual la cantidad de pedido no se toma de él sino de la rejilla calibrada (Sección~\ref{sec:newsvendor_operativo}).
