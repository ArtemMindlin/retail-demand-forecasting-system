...
\subsection{Sintonización Guiada por Coste Logístico y Validación Temporal}
\label{sec:tuning_metodologia}
\label{sec:pareto}

Históricamente, los sistemas de predicción de demanda sintonizan sus hiperparámetros minimizando funciones de error simétrico promedio (como el MAE o el RMSE). Sin embargo, en un entorno de inventario con costes fuertemente asimétricos, una mejora en el error cuadrático medio no se traduce necesariamente en una reducción de los costes operativos reales.

Para resolver este desacoplamiento, el sistema implementa una \textbf{Sintonización de Hiperparámetros Guiada por Coste Logístico} (\textit{Cost-Driven Hyperparameter Optimization}) utilizando \textit{Optuna} \cite{akiba2019optuna} y el algoritmo TPE (\textit{Tree-structured Parzen Estimator}). En lugar de evaluar los candidatos con una función de pérdida abstracta, cada configuración de hiperparámetros se somete al flujo operacional completo: se ajustan los modelos multi-cuantil, se calibran sus residuos mediante la capa de \textit{Mondrian Conformal Prediction} (Sección~\ref{sec:conformal}) y se calcula el coste económico total del modelo \textit{Newsvendor} ($c_s \cdot \text{rotura} + c_h \cdot \text{almacenamiento}$) sobre la ventana de evaluación.
...
\begin{enumerate}
    \item \textbf{Ventana de Selección ($N=30$ extracciones temporales):} Optuna explora el espacio de búsqueda sobre un conjunto histórico cerrado, guiando la propuesta de candidatos hacia configuraciones que minimizan el coste logístico medio simulado.
    \item \textbf{Ventana de Validación \textit{Out-of-Sample} ($N=25$ extracciones independientes posteriores):} La configuración con menor coste de selección no se adopta automáticamente. Se somete a una \textbf{puerta de validación} (\textit{validation gate}) donde compite contra los hiperparámetros por defecto sobre una ventana temporal futura genuinamente no observada. Se calcula un intervalo de confianza \textit{bootstrap} al 95\% sobre la diferencia pareada de costes: solo si la mejora es estadísticamente significativa ($\text{IC}_{95\%}^{\text{sup}} < 0$), los nuevos parámetros son persistidos en \texttt{models/forecasting\_params.json} para su uso en el \textit{backtesting} global.
\end{enumerate}

\subsubsection{Evaluación de la Calidad de los Intervalos: Winkler Score}
\label{sec:winkler}
Junto al coste logístico de inventario, el sistema requiere evaluar la calidad intrínseca de los intervalos de incertidumbre producidos por la distribución predicha. Para ello se adopta el \textbf{Winkler Score} \cite{winkler1972scoring}, una regla de puntuación estrictamente propia (\textit{strictly proper scoring rule}) que penaliza simultáneamente la amplitud del intervalo (\textit{nitidez}) y los fallos de cobertura en una única métrica consolidada.
...
Si en cambio el modelo produjera un intervalo trivial e hiper-conservador $[0, 100]$ unidades, cubriría siempre la demanda real pero incurriría en un pago constante de $W = 100$, muy superior al del intervalo ajustado. El Winkler Score se emplea en el módulo de evaluación final y en el cuadro de mandos (Sección~\ref{sec:metricas_evaluacion}) para auditar la fiabilidad de las bandas de incertidumbre antes y después de la calibración conformal.
