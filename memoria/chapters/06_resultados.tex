...
El módulo \texttt{LatentDemandImputer} implementa tres estrategias de imputación alternativas (véase Sección~\ref{sec:demanda_censurada}). La Tabla~\ref{tab:imputation_comparison} recoge su comparativa empírica sobre 50 series con $n=100$ observaciones de evaluación, empleando LightGBM como modelo de forecasting en todos los casos. Las cifras absolutas de Winkler de esta tabla no son directamente comparables con las de la Tabla~\ref{tab:conformal_metrics}: proceden de un \textit{run} distinto, con otro subconjunto de evaluación y otro tamaño muestral, por lo que solo deben leerse en términos relativos \emph{dentro} de la tabla.

\begin{table}[h]
\centering
\small
\caption{Comparativa de estrategias de imputación de demanda latente (LightGBM, $n=100$). La última columna expresa la mejora relativa en Winkler Score respecto a la estrategia sin corrección.}
\label{tab:imputation_comparison}
\begin{tabular}{lrrr}
\toprule
\textbf{Estrategia} & \textbf{MAE} & \textbf{Winkler Score} & \textbf{Mejora Winkler vs.\ Observed} \\
\midrule
Observed (sin corrección) & 30{,}92 & 290{,}96 & n/a \\
...
\begin{itemize}
    ...
    \item \textbf{Las conclusiones logísticas se mantienen parcialmente}: en la simulación dinámica Order-Up-To a 500 series, CatBoost Latent reduce el coste simulado un 3{,}9\% frente al Seasonal Naïve Latent (426.215 frente a 443.289 u.m.) y eleva el nivel de servicio del 79{,}3\% al 94{,}4\%. La corrección de censura mantiene su efecto positivo sobre el \textbf{coste estático} en todos los modelos (CatBoost: 128.868 $\rightarrow$ 120.023 u.m., $+$6{,}9\%; Seasonal Naïve: 143.819 $\rightarrow$ 119.958 u.m., $+$16{,}6\%). Sin embargo, en el \textbf{coste dinámico simulado} la relación se invierte: CatBoost Observed (410.761 u.m.) resulta un 3{,}6\% más barato que CatBoost Latent (426.215 u.m.). El beneficio de la corrección de censura sobre la política dinámica queda acotado a las series de mayor severidad de stockout, sobrerrepresentadas en el subset base de 50 series pero diluidas a mayor escala. Esta inversión no invalida la corrección de censura como práctica --- el efecto sobre el coste estático es consistente y positivo en todos los modelos, y el nivel de servicio sube de forma sustancial --- sino que delimita en qué escenarios (catálogos dominados por stockouts severos) el impacto dinámico es más pronunciado.
\end{itemize}

El \textbf{Winkler Score} de LightGBM (27,55 a escala; 22,35 en el subset base) es inferior al de CatBoost (30,91 y 24,40 respectivamente), lo que indica intervalos más eficientes (más estrechos con mayor cobertura). Sin embargo, el criterio de selección del modelo campeón es el coste logístico simulado, no el Winkler Score. En la simulación dinámica \textit{Order-Up-To} a escala (500 series) con estrategia \textit{Latent Supervised}, CatBoost logró el menor coste simulado (426.215 u.m.), dominando a LightGBM (433.043 u.m.) y al \textit{Seasonal Naïve} (443.289 u.m.), lo que justifica su elección como modelo campeón. La Figura~\ref{fig:mae_vs_coste} sintetiza esta disociación entre error puntual y coste logístico, que constituye el resultado central del trabajo: el orden de los modelos por MAE es exactamente el inverso de su orden por coste simulado.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/mae_vs_coste.pdf}
    \caption{Disociación entre el error puntual y el coste logístico en la validación a escala (500 series, estrategia \textit{Latent Supervised}): el \textit{Seasonal Naïve} obtiene el mejor MAE del backtesting predictivo pero el peor coste en la simulación dinámica \textit{Order-Up-To}, mientras que CatBoost presenta el patrón inverso. Optimizar el error puntual no equivale a optimizar la política de inventario.}
    \label{fig:mae_vs_coste}
\end{figure}
...
un criterio de validez --- la garantía del Split Conformal no se sostiene sobre una
calibración que el modelo ha visto --- y no por optimización de métricas.
...
\begin{itemize}
    \item \textbf{Política de pedido común y simplificada}: la cantidad a pedir no proviene de los cuantiles del modelo de \textit{forecasting} ni de la capa conformal, sino de una aproximación normal \textit{order-up-to} idéntica para todas las estrategias, $q^* = \max(\text{señal} + z_{q^*}\cdot\sigma,\, 0)$, donde $z_{q^*}$ es el cuantil normal estándar del fractil crítico. Esta comparativa mide por tanto el \textbf{imputador}, no el modelo campeón; el rendimiento del sistema completo se evalúa en la simulación dinámica de la Sección~\ref{sec:resultados_escala}.
...
Los resultados de esta comparativa equitativa, calculada sobre un subconjunto de 30 series muestreadas del panel de 500 y $n = 2\,018$ días censurados sintéticamente, se presentan en la Tabla~\ref{tab:metrics_cost}.
...
extraen}. Muestreadas del panel de 500 series se reproduce el orden de la
Tabla~\ref{tab:metrics_cost}: \textit{Latent Supervised} resulta la estrategia más
barata (1.774{,}82 frente a 1.843{,}41 u.m.\ de \textit{Observed} en una reejecución
de control, un $-3{,}7\%$). Muestreadas en cambio del subset base de 50 series de alta
rotación, el orden \textbf{se invierte} y \textit{Observed} pasa a ser la más barata
(844{,}01 frente a 927{,}80 u.m.). La causa es que el subset base contiene demasiadas
pocas series con regímenes de rotura severa para que la corrección emerja por encima
del ruido de muestreo con $n=30$: precisamente el mecanismo por el que la ventaja se
diluye a escala, aquí operando en sentido contrario. La conclusión de esta sección debe
leerse por tanto acotada al panel de 500 series, que es el que declara la columna
\texttt{source\_panel\_series} del artefacto \texttt{fair\_cost\_backtest.csv}, y no
como una propiedad independiente del subconjunto evaluado. Ampliar $n$ hasta que el
signo sea estable frente al remuestreo queda como verificación pendiente.
\end{quote}
...
donde $q_i$ es la cantidad a pedir del SKU $i$, $d_i$ la cantidad óptima sin restricción (Newsvendor individual, que actúa como cota superior), $u_i$ la utilidad marginal de cada unidad y $C$ la capacidad global del almacén. El solver empleado es \texttt{scipy.optimize.linprog} con el método \texttt{highs}.

\subsection{El Coste de Asumir Utilidad Marginal Constante}
\label{sec:lp_utilidad_constante}

La formulación de la Ecuación~\ref{eq:lp_capacity} admite una lectura ingenua que
resulta ser incorrecta: tomar $u_i = c_{u,i}$, es decir, asumir que \emph{toda}
unidad del SKU $i$ evita una rotura y por tanto vale lo mismo. Bajo ese supuesto el
objetivo es lineal en $q_i$ sobre restricciones de caja, y el óptimo es
necesariamente una \textbf{solución de esquina}: el solver llena hasta su cota $d_i$
los SKUs de mayor $c_{u,i}$ y asigna \emph{cero} al resto. No es un defecto del
solver, sino la respuesta matemáticamente correcta a un objetivo mal especificado.

La consecuencia empírica es severa. Comparada contra el recorte proporcional ciego
--- el mecanismo de \textit{fallback} del propio módulo --- esta formulación resulta
\emph{peor} en todos los regímenes de capacidad evaluados, entre un 30\% y un 163\%
más de penalización por rotura (Tabla~\ref{tab:lp_sweep}). El motivo es que los
pedidos Order-Up-To incorporan stock de seguridad y cobertura de \textit{lead time},
lo que les da un holgura media de $2{,}18\times$ sobre la demanda finalmente
realizada: un recorte uniforme del 25\% deja a todos los SKUs cubiertos, mientras que
anular por completo 52 de las 150 decisiones garantiza otras tantas roturas.

\subsection{Formulación Correcta: Valor Marginal Decreciente}
\label{sec:lp_tramos}

El valor de una unidad no es constante: vale $c_{u,i}$ solo mientras cubra demanda que
de otro modo se perdería, y a partir de ahí genera coste de almacenamiento. Derivando
el coste esperado del Newsvendor, el valor marginal de la unidad situada en la
posición $q$ es
\begin{equation}
    \frac{\partial V_i}{\partial q} \;=\; c_{u,i} \;-\; (c_{u,i} + c_{o,i})\, F_i(q),
    \label{eq:lp_marginal}
\end{equation}
positivo únicamente mientras $F_i(q)$ se mantenga por debajo del fractil crítico
$c_{u,i}/(c_{u,i}+c_{o,i})$, que es exactamente la condición del Newsvendor de la
Sección~\ref{sec:newsvendor}. La función de valor es por tanto \textbf{cóncava}, y su
aproximación lineal por tramos sigue siendo resoluble como un LP: basta introducir una
variable por cada par (SKU, tramo), acotada por la anchura del tramo, y dejar que el
solver llene primero las unidades globalmente más valiosas.

La distribución $F_i$ no requiere ningún supuesto paramétrico adicional: se lee
directamente de los cuantiles que el modelo ya predice, tomando el nivel medio de cada
intervalo como probabilidad representativa del tramo. Los tramos situados más allá del
fractil crítico reciben valor negativo y el solver los deja vacíos por sí mismo, sin
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrr}
        \toprule
        \textbf{Capacidad} & \textbf{LP por tramos} & \textbf{LP utilidad const.} & \textbf{Series anuladas (tramos / const.)} \\
        \midrule
        75\% & $-11{,}7\%$ & $-160{,}1\%$ & 9 / 52 \\
        60\% & $-1{,}0\%$  & $-163{,}4\%$ & 20 / 79 \\
        50\% & $\mathbf{+2{,}5\%}$ & $-157{,}8\%$ & 26 / 95 \\
        40\% & $\mathbf{+2{,}7\%}$ & $-111{,}5\%$ & 39 / 110 \\
        30\% & $\mathbf{+3{,}5\%}$ & $-69{,}9\%$  & 59 / 121 \\
        20\% & $\mathbf{+2{,}7\%}$ & $-30{,}5\%$  & 78 / 131 \\
        \bottomrule
    \end{tabular}
    \caption{Reducción de la penalización por rotura frente al recorte proporcional ciego, según la capacidad disponible expresada como fracción de las unidades solicitadas sin restricción (subset base de 50 series, CatBoost \textit{Latent Supervised}). Valores positivos indican que la asignación bate al recorte proporcional. Ambas políticas consumen exactamente la misma capacidad, por lo que el coste de almacenamiento es idéntico y la penalización por rotura es el único diferenciador. Reproducible con \texttt{scripts/capacity\_lp\_experiment.py}.}
    \label{tab:lp_sweep}
\end{table}

La formulación por tramos revierte el signo del resultado: bate al recorte
proporcional en los cuatro niveles en los que la capacidad restringe de forma
apreciable ($\leq 50\%$), con reducciones del 2{,}5\% al 3{,}5\%, y reduce las
decisiones anuladas de 52 a 9 en el escenario más holgado.

Conviene precisar por qué \emph{pierde} con capacidad holgada (75\% y 60\%), porque no
se trata de un defecto de la formulación. Con esos presupuestos la holgura de los
pedidos Order-Up-To es suficiente para que repartir uniformemente cubra la demanda
realizada de todos los SKUs y no produzca ninguna rotura. El LP optimiza el coste
\emph{esperado} bajo la distribución predicha, no el realizado, y no puede saber
\textit{ex ante} que sobraba capacidad; el recorte proporcional se beneficia aquí de
una ventaja puramente \textit{ex post} que ninguna política prospectiva puede
explotar. La conclusión operativa es por tanto acotada pero honesta: la asignación por
programación lineal aporta valor cuando la restricción de capacidad es genuinamente
severa, y es indistinguible del reparto proporcional cuando no lo es.

Las magnitudes son además modestas (2--4\%), muy lejos de lo que sugeriría el
razonamiento cualitativo de priorizar los SKUs de rotura más cara. La razón es que en
este panel los costes de rotura son casi homogéneos ($c_u$ entre 3{,}70 y 3{,}96 u.m.),
de modo que hay poca señal sobre la que discriminar. Un catálogo con perfiles de coste
más heterogéneos ---productos de alta caducidad frente a bienes de larga rotación---
es el escenario en el que esta priorización debería rendir de forma apreciable, y su
verificación queda como línea de trabajo futuro.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{5.5cm}>{\centering\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{4.5cm}c}
        ...
        \midrule
        Corrección de censura mejora el coste vs.\ estrategia Observed & $> 0\%$ & $-4{,}81\%$ (Latent Supervised vs.\ Observed, coste justo) & \checkmark \\
        \midrule
        CatBoost supera al Seasonal Naïve en coste logístico & $> 0\%$ & $-3{,}9\%$ (Latent, coste simulado a escala, 500 series) & \checkmark \\
        \midrule
    ...
    \caption{Verificación de los KPIs del trabajo. Las tres primeras filas se corresponden con los objetivos \ref{obj:pipeline}, \ref{obj:modelos} y \ref{obj:conformal} de la Sección~\ref{sec:objetivos}. \checkmark = cumplido; Parcial = cumplido con matices; Limitado = no alcanzado en el subset de evaluación. $^{\dagger}$El campeón se selecciona por coste logístico simulado, no por Winkler Score; entre los modelos probabilísticos, LightGBM obtiene el mejor Winkler (27,55) pero CatBoost domina en coste simulado a escala (426.215 u.m. frente a 433.043 u.m. de LightGBM). $^{\ddagger}$Este criterio no figura entre los objetivos específicos del Capítulo~\ref{chap:introduccion}: procede del protocolo de evaluación de la Sección~\ref{sec:backtesting}, que establece el Winkler Score frente al \textit{Seasonal Naïve} como umbral mínimo de aportación de valor probabilístico. Se incluye aquí por completitud.}
    \label{tab:kpi_verification}
\end{table}

Los dos primeros objetivos (el impacto de la corrección de censura y la superioridad en coste del modelo probabilístico) se cumplen con margen. El tercero se cumple solo parcialmente: el Seasonal Naïve no genera intervalos, por lo que la comparación de Winkler Score se restringe a los modelos probabilísticos; entre ellos, LightGBM obtiene el mejor valor (27,55), aunque el campeón se selecciona por coste logístico simulado. Como se adelantó en la Sección~\ref{sec:resultados_escala}, bajo ese criterio CatBoost domina claramente a LightGBM (426.215 u.m. frente a 433.043 u.m.) en la simulación dinámica a escala. El cuarto objetivo ($\geq 80\%$ de cobertura) no se alcanza: en el subset base por insuficiencia del conjunto de calibración por estrato, y en la validación a 500 series (Sección~\ref{sec:resultados_escala}), que eleva la cobertura al 68{,}3\% y estrecha la inestabilidad entre folds, por la no-intercambiabilidad temporal entre la ventana de calibración y el test, limitación estructural del Split CP sobre paneles de 90 días que motiva la adopción de CP adaptativa como línea de trabajo futuro.

\section{Caso de Uso Ilustrativo: Mecánica de la Política Order-Up-To paso a paso}
\label{sec:caso_uso_sS}

Para consolidar la comprensión del gemelo digital, esta sección desarrolla un ejemplo numérico sobre un SKU ficticio que reproduce la mecánica de costes implementada en \texttt{simulate\_inventory\_policy}. El bucle de simulación procesa cada periodo en dos fases: primero emite el pedido evaluando la Posición de Inventario ($IP$) con el estado inicial del día, y luego ejecuta el \texttt{step} físico (llegan los pedidos, se salda el backlog, se atiende la demanda y se computa el coste: $\text{coste} = OH_{\text{fin}}\cdot c_o + BL_{\text{fin}}\cdot c_u$). El objetivo es hacer explícita la diferencia entre la política probabilística (que conoce la distribución de la demanda) y la política puntual (que solo utiliza la media predicha).

\subsection*{Parámetros y Nomenclatura del ejemplo}

Para modelar el estado del inventario a lo largo del tiempo, empleamos la nomenclatura estándar de la literatura logística, que se corresponde directamente con los atributos de la clase \texttt{InventoryState} del simulador:
\begin{itemize}
    \item $OH$ (\textit{On-Hand}): Stock físico disponible en el almacén al inicio o fin del periodo.
    \item $OO$ (\textit{On-Order}): Unidades en tránsito que ya han sido pedidas pero aún no han llegado.
    \item $BL$ (\textit{Backlog}): Demanda insatisfecha acumulada (pedidos pendientes de entrega).
    \item $IP$ (\textit{Inventory Position}): Posición neta de inventario sobre la que se toman las decisiones ($IP = OH + OO - BL$).
    \item $S$: Nivel objetivo de inventario (\textit{Order-Up-To level}).
    \item $Q$: Cantidad a pedir emitida al proveedor en el periodo actual ($Q = S - IP$).
\end{itemize}

Los parámetros de simulación fijados para este SKU ficticio son:
\begin{itemize}
    \item Coste de sobrestock: $c_o = 1$\,u.m./unidad.
    \item Coste de rotura: $c_u = 4$\,u.m./unidad.
    \item \textit{Lead time}: 7 días (el pedido emitido hoy llega al inicio del siguiente periodo semanal).
    \item Estado inicial (Periodo 0): $OH_0 = 0$, $BL_0 = 0$, $OO_1 = 0$.
\end{itemize}
...
\end{equation}
El sistema debe por tanto cubrir el percentil 80 de la distribución de demanda.

\subsection*{Periodo 1: primer pedido y demanda}

El modelo probabilístico predice una demanda para el Periodo 1 donde $q_{0{,}80} \approx 72$. La política puntual predice la mediana $\hat{y}_1 = 50$.

\medskip
\noindent\textbf{1. Emisión del pedido (Inicio del periodo):}
La posición de inventario es $IP_1 = OH_0 - BL_0 + OO_1 = 0 - 0 + 0 = 0$.
\begin{itemize}
    \item Probabilística: Pide $Q_1 = 72 - 0 = 72$ unidades.
    \item Puntual: Pide $Q_1 = 50 - 0 = 50$ unidades.
\end{itemize}
Estos pedidos llegarán al inicio del Periodo 2.

\medskip
\noindent\textbf{2. Resolución física (Fin del periodo):}
Llegan 0 unidades. La demanda real es $y_1 = 55$.
\begin{align*}
    OH_1 &= 0 + 0 \text{ (arrivals)} = 0 \\
    \text{servido} &= \min(0,\; 55) = 0 \\
    BL_1 &= 55 - 0 = 55 \text{ unidades (rotura total)} \\
    \text{coste}_1 &= 0 \times 1 + 55 \times 4 = 220\,\text{u.m.} \quad \text{(para ambas políticas)}
\end{align*}

\subsection*{Periodo 2: llegada del pedido}

El modelo predice para el Periodo 2 un $S_2 = 70$ (probabilística) y un $\hat{y}_2 = 50$ (puntual).

\medskip
\noindent\textbf{1. Emisión del pedido (Inicio del periodo):}
El estado arrastrado es $OH = 0$, $BL = 55$. Los pedidos del Periodo 1 pasan a ser "llegadas" inminentes, por lo que ya no cuentan como \textit{On-Order} futuro ($OO_2 = 0$).
$IP_2 = OH - BL + OO_2 = 0 - 55 + 0 = -55$.
\begin{itemize}
    \item Probabilística: $Q_2 = S_2 - IP_2 = 70 - (-55) = 125$ unidades.
    \item Puntual: $Q_2 = 50 - (-55) = 105$ unidades.
\end{itemize}
El \textit{backlog} arrastrado infla automáticamente el nuevo pedido.

\medskip
\noindent\textbf{2. Resolución física (Fin del periodo):}
Llegan los pedidos del Periodo 1 (72 y 50). La demanda real es $y_2 = 60$.

\noindent\textbf{Política probabilística:}
\begin{align*}
    OH_2 &= 0 + 72 \text{ (arrivals)} = 72 \\
...
\begin{align*}
    OH_2 &= 0 + 50 \text{ (arrivals)} = 50 \\
    \text{backlog saldado} &= \min(50,\; 55) = 50 \Rightarrow OH_2 = 0,\; \text{BL restante} = 5 \\
...
Asumimos una predicción idéntica ($S_3 = 70$, $\hat{y}_3 = 50$).

\medskip
\noindent\textbf{1. Emisión del pedido:}
\begin{itemize}
    \item Probabilística ($BL=43$): $IP_3 = 0 - 43 = -43 \Rightarrow Q_3 = 70 - (-43) = 113$ unidades.
    \item Puntual ($BL=65$): $IP_3 = 0 - 65 = -65 \Rightarrow Q_3 = 50 - (-65) = 115$ unidades.
\end{itemize}

\medskip
\noindent\textbf{2. Resolución física:}
Llegan los pedidos del Periodo 2 (125 y 105). La demanda es $y_3 = 50$.

\noindent\textbf{Política probabilística:}
\begin{align*}
    OH_3 &= 0 + 125 = 125 \\
    \text{backlog saldado} &= \min(125,\; 43) = 43 \Rightarrow OH_3 = 82,\; BL_3 = 0 \\
    \text{servido}_3 &= \min(82,\; 50) = 50 \\
    OH_3 &= 32,\; BL_3 = 0 \\
    \text{coste}_3 &= 32 \times 1 + 0 \times 4 = 32\,\text{u.m.}
\end{align*}

\noindent\textbf{Política puntual:}
\begin{align*}
    OH_3 &= 0 + 105 = 105 \\
    \text{backlog saldado} &= \min(105,\; 65) = 65 \Rightarrow OH_3 = 40,\; BL_3 = 0 \\
    \text{servido}_3 &= \min(40,\; 50) = 40 \\
    OH_3 &= 0,\; BL_3 = 10 \\
    \text{coste}_3 &= 0 \times 1 + 10 \times 4 = 40\,\text{u.m.}
\end{align*}

\subsection*{Comparación acumulada de costes}

La Tabla~\ref{tab:caso_uso_sS} resume el coste de inventario acumulado por ambas políticas a lo largo de los tres periodos simulados.
