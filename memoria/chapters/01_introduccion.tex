...
\begin{enumerate}
    \item \textbf{El protocolo de censura sintética no ordena reglas de reconciliación, y se
    cuantifica por qué.} Su verdad-terreno se construye como
    $\text{observado} = \text{real}\cdot(1-r)$, de modo que toda regla cuya forma funcional
    invierta esa relación acierta por construcción: una que solo use el ratio de rotura $r$, sin
    modelo alguno, obtiene el menor error de reconstrucción de todas las evaluadas. De ahí que
    las estrategias se ordenen aquí por coste y no por error de reconstrucción, y que una
    comparación de hiperparámetros sobre el mismo protocolo siga siendo válida, porque todos los
    candidatos comparten una regla y el supuesto del generador se cancela
