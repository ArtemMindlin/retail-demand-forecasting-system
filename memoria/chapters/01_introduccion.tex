...
\begin{enumerate}
    ...
    circular, y se cuantifica cuánto.} Medir un imputador exige conocer la demanda real de un día
    con rotura, que es justo lo que nunca se observa. El protocolo habitual lo resuelve
    falsificándola: toma días \emph{sin} rotura, donde la demanda real sí se conoce, y finge que
    la tuvieron multiplicándola por $(1-r)$, siendo $r$ la fracción de la jornada sin producto.
    Un día de 10 unidades con media jornada vacía queda anotado como una venta de 5, y esas 10
    unidades pasan a ser la respuesta contra la que se puntúa.

    El problema es que esa relación se despeja: $\text{real} = \text{observado}/(1-r)$, y $r$
    está anotado en los datos. Una regla que se limite a dividir por $(1-r)$ recupera la
    respuesta \textbf{exactamente}, sin modelo y sin entrenar nada, mientras que un imputador que
    estime la demanda de jornada completa se queda cerca pero no clavado. Medido sobre el panel
    de este trabajo, esa regla trivial obtiene el menor error de reconstrucción de todas las
    estrategias evaluadas.

    Y no por ello es la mejor forma de reconstruir demanda, sino la peor de las cuatro por el
    motivo que importa: la proporcionalidad $\text{observado} = \text{real}\cdot(1-r)$ es un
    supuesto del simulador, no un hecho del mundo. En una rotura real no se pierde exactamente la
    fracción de demanda correspondiente a la fracción de jornada sin producto. Parte de los
    clientes vuelve más tarde, parte sustituye el producto y parte se va a otro establecimiento, y
    la demanda tampoco se reparte de forma uniforme a lo largo del día, de modo que una rotura en
    hora punta y otra en hora muerta no cuestan lo mismo. Esa regla acierta sobre
    los días falsificados porque los falsificó ella misma, y no tiene ninguna garantía sobre los
    días censurados de verdad. El error de reconstrucción no mide, por tanto, la calidad de la
    reconstrucción: mide el parecido entre la fórmula del imputador y la fórmula con la que se
    falsificaron los datos.

    De ahí las dos consecuencias que recorren esta memoria. Las estrategias de imputación se
    ordenan por el coste de inventario que inducen frente a una verdad común, nunca por su error
    de reconstrucción (Sección~\ref{sec:imputacion_censura_sintetica}). Y la búsqueda de
    hiperparámetros del imputador sí es válida sobre este mismo protocolo, porque todos sus
    candidatos comparten una única fórmula: la circularidad les afecta por igual y se cancela al
    compararlos entre sí.

    \item \textbf{Los hiperparámetros del imputador no transfieren entre tamaños de ajuste, y el
    efecto cambia de signo.} La búsqueda mejora al ajuste por defecto con pocos cientos de filas
    limpias y lo empeora con decenas de miles, que es la escala a la que el sistema opera. La
    tendencia es monótona en el tamaño de ajuste y se confirma sobre dos periodos de puntuación
    distintos, de modo que es un efecto de escala y no de calendario. Un fichero de
    hiperparámetros solo es válido cerca del tamaño con el que se obtuvo, condición que el
    artefacto registra explícitamente (Anexo~\ref{anx:tuning_imputador}).
