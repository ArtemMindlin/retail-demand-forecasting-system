...
desperdicio, con un umbral de decisión calculado producto a producto según su propio perfil de
costes, contribuyendo así al Objetivo de Desarrollo Sostenible (ODS) 12: Producción y Consumo
...
\begin{itemize}
    ...
    \item \textbf{Responsable de tienda u operaciones} (lectura agregada). Dispone de una medida
    del nivel de servicio y del coste por producto y por categoría, lo que permite localizar dónde
    se concentran las roturas y las mermas en lugar de diagnosticarlas producto a producto.
...
Las aportaciones originales de este trabajo, diferenciadas del estado del arte revisado en el Capítulo~\ref{chap:estado_del_arte} y justificadas frente a los trabajos previos en la Sección~\ref{sec:posicionamiento}, son las siguientes:
...
\begin{enumerate}
    ...
    \item \textbf{Sistema de Apoyo a la Decisión con cierre logístico.} La mayoría de los trabajos académicos evalúan el rendimiento predictivo mediante métricas estadísticas (MAE, RMSE) desconectadas de la operativa real. Este trabajo cierra el ciclo integrando el modelo probabilístico con la política de pedido que se deriva de él, y midiendo el resultado en las dos magnitudes que la operación percibe: el coste logístico y el nivel de servicio. Esa doble lectura resulta ser imprescindible, porque ambas no ordenan igual a los modelos.

    \item \textbf{Selección de hiperparámetros gobernada por la decisión.} La búsqueda de hiperparámetros no se dirige a un error estadístico de propósito general, sino a las dos magnitudes de las que depende la decisión de reposición: la \textit{Pinball Loss} evaluada en el fractil crítico $q^*$ que fija la política de inventario, y el Winkler Score, que mide la calidad del intervalo sobre el que se cuantifica la incertidumbre. Ambas se mantienen separadas durante la búsqueda, de modo que su resultado no es una configuración aislada sino un frente de Pareto que hace explícito el compromiso entre ellas y permite reponderarlo ante un cambio en la estructura de costes sin repetir la exploración. La aportación no reside en el empleo de optimización multiobjetivo, técnica consolidada y disponible en las librerías al uso, sino en derivar sus objetivos del problema de decisión al que el modelo sirve.
...
\begin{enumerate}
    ...
    con compuertas de calidad que rechazan la ejecución si el panel no cumple sus condiciones
...
La memoria se completa con dos anexos. El \textbf{Anexo~\ref{anx:ods}} analiza la
vinculación del trabajo con los Objetivos de Desarrollo Sostenible de la Agenda 2030, y el
\textbf{Anexo~\ref{anexo:dashboard}} recoge las capturas del cuadro de mandos web, que
documentan visualmente las vistas descritas en el Capítulo~\ref{chap:implementacion}.
