...
de backtesting y el mismo envoltorio cuantílico. XGBoost se descarta antes de implementarse, y
conviene ser preciso sobre el motivo, porque no es una limitación de la herramienta: desde su
versión 1.5 soporta variables categóricas de forma nativa, con un algoritmo de particionamiento
propio para las divisiones categóricas, de modo que el \textit{one-hot} manual dejó de ser
necesario. La razón es de alcance y de redundancia. Sobre el eje que más pesa en este dominio, la
forma de codificar identificadores de cardinalidad alta, XGBoost y LightGBM comparten mecanismo:
ambos buscan la partición ordenando las categorías por el estadístico del gradiente. Un tercer
\textit{backend} que replica el mecanismo de uno de los dos ya implementados multiplicaría cada
corrida de backtesting sin añadir un contraste nuevo, mientras que CatBoost es el único de los
tres que aporta un mecanismo distinto, los estadísticos de destino ordenados.
...
\begin{itemize}
    ...
    Es la única de las dos que introduce la etiqueta en la codificación y, a la vez, la única que
    incorpora un mecanismo explícito para que hacerlo no filtre información. Con identificadores de
    la cardinalidad de este catálogo, esa diferencia es la más relevante de la tabla.
