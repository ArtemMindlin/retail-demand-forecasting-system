...
class LatentDemandImputer:
    ...
    def __init__(
        self, 
        strategy: Literal["supervised", "historical_mean", "clipped_scaling", "none"] = "supervised",
    ...
    def _impute_supervised(self, df: pd.DataFrame, is_clean: pd.Series, is_censored: pd.Series) -> pd.DataFrame:
    ...
    def _impute_historical_mean(self, df: pd.DataFrame, is_clean: pd.Series, is_censored: pd.Series) -> pd.DataFrame:
    ...
    def _impute_clipped_scaling(self, df: pd.DataFrame, is_clean: pd.Series, is_censored: pd.Series) -> pd.DataFrame:
        """Baseline: Simply scale up the observed demand by a fixed factor during stockouts."""
        df.loc[is_censored, "latent_demand_est"] = df.loc[is_censored, self.target_col] * self.scaling_factor
        df.loc[is_clean, "latent_demand_est"] = df.loc[is_clean, self.target_col]
        return df
