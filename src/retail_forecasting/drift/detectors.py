...
class PageHinkleyDetector:
    ...
    def __init__(self, threshold: float = 30.0, delta: float = 0.005, min_instances: int = 30):
    ...
    def update(self, error: float) -> DriftResult:
        ...
        if self.n > self.min_instances and ph_stat > self.threshold:
            self.is_drift = True
            return DriftResult(is_drift=True, score=ph_stat, threshold=self.threshold, detected_at_index=self.n)
