...
REPO_MEMORIA_DIR = Path(__file__).resolve().parents[1] / "memoria"
...
@pytest.fixture(autouse=True)
def _memoria_dir_never_points_at_the_repo(tmp_path_factory, monkeypatch) -> None:
    """Redirect the default thesis directory away from the repo's own ``memoria/``.

    Same backstop as the one above, for the same reason and after the same accident: two tests
    that exercised ``run_eda`` without naming a directory rendered sixteen figures from a
    3-series synthetic panel and copied twelve of them over the real thesis figures. Those are
    gitignored, so nothing could restore them -- they had to be regenerated by a full EDA run.
    Tests that mean to export should pass ``memoria_dir`` explicitly, as they already do.
    """
    sandbox = tmp_path_factory.mktemp("memoria_dir_default")
    field = ReportingConfig.model_fields["memoria_dir"]
    monkeypatch.setattr(field, "default", sandbox)
