...
- `src/retail_forecasting/forecasting/`: walk-forward validation, conformal calibration, imputation comparison, fair-cost backtesting, and experiment/retrain/scoring orchestration (`pipeline.py`); `imputation_tuning.py` runs a separate Optuna search over the supervised imputer's LGBM hyperparameters and persists the winner (never fitted weights) to `models.models_dir/imputation_lgbm_params.json` — its flow, draws and persist gates are mapped in `src/retail_forecasting/forecasting/imputation_tuning.md`. It logs to MLflow (`mlflow.db`, browsable via `make mlflow-ui`), as does every experiment run through `evaluation/tracking.py`: metrics, config params, decisions and figures, so `mlflow.search_runs` can compare runs the way 92 timestamped folders cannot. The row-level outputs stay under `reports/`, which the dashboard and `utils/latex_exporter.py` read — `predictions.csv` alone is 1.9 GB across runs and MLflow's artifact store is a plain directory, so moving it would put the same bytes behind a UUID.
