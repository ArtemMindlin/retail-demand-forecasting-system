...
- `mlruns/` + `mlflow.db`: the run store. Every mode opens an MLflow run and writes its artifacts straight into that run's directory, so there is one copy and no mirroring step. Both are gitignored, which is why each artifact directory carries an `mlflow_run.json` naming its run: with a database-backed store, `mlruns/` holds artifacts and nothing that identifies them. The artifact location MLflow bakes into each experiment row is RELATIVE, so the store resolves against the working directory and the same `mlflow.db` works both in the repo and mounted at `/app` in the container — at the price of a run launched from a subdirectory not finding its own artifacts.
...
- `src/retail_forecasting/evaluation/`: metrics, run reporting, post-mortem analysis, XAI/explainability, and `latex_exporter.py`, which renders the generated tables of chapter 6. It sits here rather than in `utils` because it resolves a run by the name `docs/runs.md` cites, and reaching the run index needs `tracking` — which `utils`, importing no first-party layer, cannot have without closing a cycle.
