...
\begin{frame}{El Problema Logístico: Asimetría y Desperdicio}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \vspace{0.15cm}
      \centering
      \includegraphics[width=\linewidth,height=0.28\textheight,keepaspectratio]{fresh_produce.jpg}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{Tabla Comparativa: Backtesting Walk-Forward ($K=3, H=7$)}
  \begin{table}[h]
    ...
    \begin{tabular}{lcccccc}
      \toprule
      \textbf{Modelo} & \textbf{MAE} & \textbf{RMSE} & \textbf{Winkler (80\%)} & \textbf{Coste (u.m.)} & \textbf{Nivel Servicio} & \textbf{Cobertura (obj. 80\%)} \\
      \midrule
...
\begin{frame}{Optimización Biobjetivo y Estabilidad Fuera de Muestra}
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      \centering
      \includegraphics[width=\linewidth,keepaspectratio]{tuning_comparison_60trials.pdf}
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{statbox}{Protocolo Desacoplado}{UPVNavy}
        \begin{itemize}\scriptsize
          \item \textbf{60 Ensayos MOTPE (Optuna):} Coste Newsvendor vs Winkler Score.
          \item \textbf{Out-of-sample ($N=25$):} LightGBM $-22{,}7\%$, CatBoost $-10{,}0\%$ vs defaults.
        \end{itemize}
      \end{statbox}
      \vspace{0.08cm}
      \begin{statbox}{Matiz Metodológico}{AmberWarning}
        \begin{itemize}\scriptsize
          \item \textbf{CatBoost gana aquí (+12,0\%):} Más estable ($23{,}8 \rightarrow 29{,}9$) por sus árboles simétricos.
          \item \textbf{El campeón es LightGBM:} La prueba soberana es el \textit{walk-forward} (500 series).
        \end{itemize}
      \end{statbox}
    \end{column}
  \end{columns}
...
\begin{frame}{Evaluación Financiera de Demanda Latente}
  \begin{statbox}{Comparativa Bajo Verdad Común ($N=30$ series, 20 sorteos sintéticos)}{EmeraldGreen}
    \centering\small
    Evaluación emparejada del coste logístico Newsvendor frente a la misma demanda real ($n=293$ días censurados).
  \end{statbox}
  \vspace{0.05cm}
  \begin{table}[h]
    \centering
    ...
    \begin{tabular}{lcccc}
      \toprule
      \textbf{Estrategia de Tratamiento} & \textbf{Coste Total (u.m.)} & \textbf{Fill Rate} & \textbf{$\Delta$ vs.\ Obs. (IC 95\%)} & \textbf{Ahorro (\%)} \\
      \midrule
      \textbf{Observed (ventas crudas)} & 3.732,37 & 60,30\% & -- & Base (0\%) \\
      \textbf{Latent Clipped Scaling} & 2.784,60 & 70,77\% & -947,77 [-973,1; -922,4] & -25,39\% \\
      \textbf{Latent Historical Mean} & 1.059,42 & 91,47\% & -2.672,95 [-2.775,9; -2.570,0] & -71,62\% \\
      \textbf{Latent Supervised (LGBM)} & \textbf{293,12} & \textbf{97,50\%} & \textbf{-3.439,25 [-3.539,6; -3.338,9]} & \textbf{-92,15\%} \\
      \bottomrule
    \end{tabular}
  \end{table}

  \begin{herocard}
    \small \textbf{Conclusión de Negocio:} Entrenar sobre demanda latente supervisada reduce el coste un \textbf{-92,15\%} ($\Delta = -3.439{,}25$ u.m.) y eleva el \textit{fill rate} del $60{,}30\%$ al $97{,}50\%$.
  \end{herocard}
\end{frame}
...
\begin{frame}{Simulación Operacional en Streaming y Drift}
  \begin{columns}[c]
    \begin{column}{0.50\textwidth}
      \centering
      \includegraphics[width=\linewidth,keepaspectratio]{cadence_cost.pdf}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{statbox}{Simulación Prequencial (41 días en streaming)}{UPVNavy}
        \begin{itemize}\scriptsize\setlength{\itemsep}{2pt}
          \item \textbf{Política `never` (estático):} Coste $105.676{,}74$ u.m. | Fill Rate $89{,}64\%$
          \item \textbf{Política `every\_7d` (reentreno semanal):} Coste \textbf{63.112{,}52 u.m.} | Fill Rate \textbf{95{,}43\%}
          \item \textbf{Ahorro Operativo:} \textbf{−40{,}28\%} ($\Delta = -42.564$ u.m.)
          \item \textbf{Latencia Media:} Apenas \textbf{5{,}55 segundos} por ciclo.
          \item \textbf{Rigor MLOps:} IC 95\% $[-57{,}5\%; -17{,}0\%]$. No concluyente al exigir $\ge 8$ orígenes independientes.
        \end{itemize}
      \end{statbox}
    \end{column}
  \end{columns}
